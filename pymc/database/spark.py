...
class Trace():
    ...
    def gettrace(self, burn=0, thin=1, chain=-1, slicing=None):
        ...
        if chain is not None:
            ...
            return self.db.rdd.map(
                    lambda x: (x[0], x[1][tname][slicing])).sortByKey().reduce(
                            reduce_helper)[1]
    ...
    def __getitem__(self, index):
        ...
        if chain is None:
            ...
            return self.db.rdd.map(
                    lambda x: (x[0], x[1][tname][index])).sortByKey().reduce(
                            reduce_helper)[1]
        else:
            if chain < 0:
            ...
            return self.db.rdd.filter(
                    lambda x: x[0] == chain).map(
                            lambda x: x[1][tname][index]).first()
    ...
    def length(self, chain=-1):
        ...
        if chain is not None:
            ...
            return self.db.rdd.filter(
                    lambda x: x[0] == chain).map(
                            lambda x: x[1][tname].shape[0]).first()
        else:
            from operator import add
            return self.db.rdd.map(
                    lambda x: x[1][tname].shape[0]).reduce(add)
    ...
    def stats(self, alpha=0.05, start=0, batches=100,
        ...
        if chain is None:
            ...
            stat['%s%s HPD interval' % (
                int(100 * (1 - alpha)), '%')] = filtered_rdd.map(
                                                    hpd_map_helper).first()
        return stat
    ...
    def calc_hpd_multchain(self, alpha=0.05):
        ...
        def hpd_reduce_helper(x, y):
            return (x[0] + y[0],
                    np.divide(
                        np.add(np.multiply(x[1], x[0]),
                            np.multiply(y[1], y[0])),
                        x[0] + y[0]))
        return self.db.rdd.map(
                lambda x: x[1][tname]).map(
                        hpd_map_helper).reduce(
                                hpd_reduce_helper)[1]
    ...
    def calc_quantile_multchain(self, quantiles):
        ...
        def quantile_reduce_helper(x, y):
            ...
            for k in x[1].keys():
                average_quantile[k] = np.divide(
                        np.add(np.multiply(x[1][k], x[0]),
                            np.multiply(y[1][k], y[0])),
                        x[0] + y[0],
                        dtype=float)
        ...
        return self.db.rdd.map(
                lambda x: x[1][tname]).map(
                        quantile_map_helper).reduce(
                                quantile_reduce_helper)[1]
    ...
    def calc_mean_std(self):
        ...
        def reduce_helper(x, y):
            ...
            weighted_mean = np.divide(
                np.multiply(x[1], x[0]) + np.multiply(y[1], y[0]),
                x[0] + y[0],
                dtype=float)

            weighted_std = np.add(
                np.multiply(np.square(x[2]), x[0] - 1),
                np.multiply(np.square(y[2]), y[0] - 1))

            weighted_std = np.add(
                weighted_std,
                np.multiply(np.square(x[1]), x[0]))

            weighted_std = np.add(
                weighted_std,
                np.multiply(np.square(y[1]), y[0]))

            weighted_std = np.subtract(
                weighted_std,
                np.multiply(np.square(weighted_mean), x[0] + y[0]))
    ...
    def multiplechain_batchsd(self, batches=5):
        ...
        def batchsd_helper(trace):
            ...
            if len(np.shape(trace)) > 1:
                ...
                return (np.reshape([t[0] for t in sd_list], dims[1:]),
                        np.reshape([t[1] for t in sd_list], dims[1:]),
                        sd_list[0][2])
            else:
                if batches == 1:
        ...
        def reduce_helper(x, y):
            ...
            weighted_mean = np.divide(
                np.multiply(x[1], x[2]) + np.multiply(y[1], y[2]),
                x[2] + y[2],
                dtype=float)

            weighted_std = np.add(
                np.multiply(np.square(x[0]), x[2] - 1),
                np.multiply(np.square(y[0]), y[2] - 1))

            weighted_std = np.add(
                weighted_std,
                np.multiply(np.square(x[1]), x[2]))

            weighted_std = np.add(
                weighted_std,
                np.multiply(np.square(y[1]), y[2]))

            weighted_std = np.subtract(
                weighted_std,
                np.multiply(np.square(weighted_mean), x[2] + y[2]))
    ...
    def batchsd(self, chain=-1, batches=5, rdd=None):
        ...
        def batchsd_helper(trace):
            ...
            if len(np.shape(trace)) > 1:
                ...
                return np.reshape([batchsd_helper(t) for t in ttrace],
                        dims[1:])
                ...
                try:
                    batched_traces = np.resize(trace,
                            (batches, len(trace) / batches))
                except ValueError:
                    # If batches do not divide evenly, trim excess samples
                    resid = len(trace) % batches
...
class Database():
    ...
    def getstate(self):
        ...
        return self.rdd.filter(
                lambda x: x[0] == last).map(
                        lambda x: x[1]['_state_']).first()
